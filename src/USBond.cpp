...
namespace PFT::Security::FixedIncome::USBond
{
    ...
    TreasuryBill::TreasuryBill(double maturity, double face_value, double discount_rate,
                               ...
                               std::string symbol, std::string name)
        : Bond(maturity, 0, face_value, 0, 0, symbol, name), // T-Bills don't have coupons
    ...
    double TreasuryBill::price()
    {
        return get_face_value() * (1 - discount_rate * get_maturity() / 360); // Assuming 360-day year
    }

    double TreasuryBill::yield_to_maturity()
    {
        double p = price();
    ...
    std::unique_ptr<Asset> TreasuryBill::clone() const
    {
    ...
    double TreasuryNote::yield_to_maturity()
    {
    ...
    std::unique_ptr<Asset> TreasuryNote::clone() const
    {
    ...
    std::unique_ptr<Asset> TIPS::clone() const
    {
        return std::make_unique<TIPS>(*this);
    }

    MunicipalBond::MunicipalBond(double maturity, double interest_rate, double face_value,
                                 double coupon_rate, int coupon_year_frequency,
                                 bool tax_exempt, std::string state,
    ...
    std::unique_ptr<Asset> MunicipalBond::clone() const
    {
